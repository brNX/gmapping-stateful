...
protected:
    ...
    std::map<int,int> particleIndextoWorkerMap;
    int remoteparticlescounter;
    int remoteworkercounter;
    zmq::socket_t * broadcastQueue;
    zmq::socket_t * remoteWorkerResponseQueue;
    zmq::socket_t * syncqueue;
    std::vector<zmq::socket_t *> workerqueues;

    int localthreadcounter;
    void localWorker(const int id, const int startindex, const int size);
    void responseThread();
    void resampleThread();
    /*******************NEW CODE *********************************/
...
#define isnan(x) (x==FP_NAN)
    ...
    void findUsedAreas(const int id, const int offset,IntPoint& minr ,IntPoint& minf,  IntPoint& maxr , IntPoint& maxf );

    /**Just scan match every single particle.
 If the scan matching fails, the particle gets a default likelihood.*/
    inline void scanMatchOld(double * plainReading) {
    ...
    inline void scanMatchLocal(double * plainReading,const int id) {

        OrientedPoint corrected;
        double score, l, s;
        score = m_matcher.optimize(corrected, m_particles[id].map, m_particles[id].pose, plainReading);
        //    it->pose=corrected;
        if (score > m_minimumScore) {
            m_particles[id].pose = corrected;
        }

        m_matcher.likelihoodAndScore(s, l, m_particles[id].map, m_particles[id].pose, plainReading);
        m_particles[id].weight += l;
        m_particles[id].weightSum += l;

        //set up the selective copy of the active area
        //by detaching the areas that will be updated
        m_matcher.invalidateActiveArea();
        m_matcher.computeActiveArea(m_particles[id].map, m_particles[id].pose, plainReading);
    ...
    inline bool resample(const double* plainReading, int adaptSize, const RangeReading* reading) {
        ...
        if (m_neff < m_resampleThreshold * m_particles.size()) {
            ...
            //send indexes to pub/sub queue
            gmapping_structs::resampleMessage rmessage;
            for(unsigned int i=0; i<m_indexes.size();i++){
                rmessage.add_resampleindexes(m_indexes[i]);
            }
            rmessage.set_resamplecounter(m_count);

            int datasize = rmessage.ByteSize();
            char dataarray[datasize];
            rmessage.SerializeToArray(dataarray,datasize);

            zmq::message_t sensordatamsg(datasize);
            memcpy(sensordatamsg.data(),dataarray,datasize);

            zmq::message_t idmessage((void*)resampleid.data(),resampleid.size(),NULL,NULL);
           ...
           for (int i = 0 ; i<remoteworkercounter;i++){
                ...
                syncqueue->recv(&message);

                /*std::cout << "resamplesync with size " << message.size() <<std::endl;
                std::string synctext(static_cast<char*>(message.data()),message.size());
            ...
            //send empty message to pub/sub queue
            zmq::message_t idmessage((void*)resampleid.data(),resampleid.size(),NULL,NULL);
            broadcastQueue->send(idmessage,ZMQ_SNDMORE);
            broadcastQueue->send(0,0);
